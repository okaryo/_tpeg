# frozen_string_literal: true

require_relative "tpeg/errors"
require_relative "tpeg/filters"
require_relative "tpeg/helpers"
require_relative "tpeg/html_escape"
require_relative "tpeg/lexer"
require_relative "tpeg/loader"
require_relative "tpeg/parser"
require_relative "tpeg/render_context"
require_relative "tpeg/template"

module Tpeg
  def self.render(source, context = {}, filters: {}, helpers: {}, loader: nil)
    Template.new(source, filters: filters, helpers: helpers, loader: loader).render(context)
  end

  def self.raw(value)
    HtmlSafeString.new(value.to_s)
  end
end
